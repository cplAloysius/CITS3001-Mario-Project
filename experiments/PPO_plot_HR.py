...
with open(filename, mode='r', newline='') as file:
    ...
    for row in reader:
        # Append n_steps and highest_reward data
        ns1.append(row["n_steps"])
        hr1.append(float(row["highest_reward"]))
...
with open(filename2, mode='r', newline='') as file:
    ...
    for row in reader:
        ...
        hr2.append(float(row["highest_reward"]))
...
for i, txt in enumerate(hr1):
    plt.annotate(f"{txt}", (r1[i], hr1[i]), textcoords="offset points", xytext=(0,10), ha='center')
for i, txt in enumerate(hr2):
